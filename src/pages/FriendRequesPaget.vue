    ...
    <div class="friend-group">
      <ul class="friend-items" v-if="active==='1'">
        ...
        <li v-else v-for="friend in status.applyFriends" :key="friend.id"
          ...
          <div class="avatar-container">
            <img @click="showUser(friend.applyUser.id)" :src="friend.applyUser.avatarUrl"
                 class="avatar">
          </div>
      ...
      <ul class="friend-items" v-else>
        ...
        <li v-else v-for="friend in status.myApplyFriends" :key="friend.id"
            class="friend-item">
          <div class="avatar-container">
            <img @click="showUser(friend.applyUserid)" :src="friend.applyUser.avatarUrl"
                 class="avatar">
          </div>
          <div @click="showUser(friend.applyUser.id)" class="friend-info">
            <div class="friend-apply">
              <p class="friend-name">{{
                  friend.applyUser.username
                }}</p>
              <p class="remark">
                {{friend.remark}}</p>
            </div>
...
const onTabChange = async (name: string) => {
  if (name === "1") {
    ...
    if(res.code === 0 ){
      status.value.applyFriends = res.data
    }
  }
  if (name === "2") {
    const res = await myAxios.get("/friends/getMyRecords")
    if(res.code === 0 ){
      console.log(res.data)
      status.value.myApplyFriends = res.data
    }
  }
}
const agree = async (id , fid: number) => {
  const agree = await myAxios.post(`/friends/agree/${id}`)
  if (agree) {
...
const canceledApply = async (id: number) => {
  const canceledApply = await myAxios.post(`/friends/canceledApply/${id}`)
  if (canceledApply) {
